...
* **Robustez na Extração de Dados (OCR):** O processo de OCR foi simplificado para focar na extração de texto de Regiões de Interesse (ROI) específicas dentro do PDF, utilizando a função `extract_text_from_roi` em `common/services.py`. A binarização de imagem agora utiliza `numpy` para maior precisão. Nunca busque por textos exatos de fontes não confiáveis como PDFs. Utilize **expressões regulares flexíveis** e funções de normalização de texto (como `normalize_text`) para contornar erros de reconhecimento de caracteres. (Ex: `re.search("CERTIFICADO DE INSPE[CÇ][AÃ]O.*?")`).
