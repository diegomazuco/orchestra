...
class Infracao(models.Model):
    ...
    placa_veiculo = models.CharField(  # type: ignore
        max_length=10, help_text="Placa do veículo que cometeu a infração."
    )
    data_hora = models.DateTimeField(help_text="Data e hora em que a infração ocorreu.")  # type: ignore
    local = models.CharField(max_length=255, help_text="Local da infração.")  # type: ignore
    tipo_infracao = models.CharField(  # type: ignore
        max_length=255, help_text="Descrição do tipo de infração."
    )
    valor = models.DecimalField(  # type: ignore
        max_digits=10, decimal_places=2, help_text="Valor da multa."
    )
    pontos_carteira = models.IntegerField(  # type: ignore
    ...
    dados_origem = models.JSONField(  # type: ignore
        help_text="Dados brutos originais do banco de dados de origem."
    )

    objects: "Manager[Infracao]"  # type: ignore
