...
### Correções de Tipagem e Refatoração
- **Ação:** Correção de problemas de tipagem e refatoração do código para maior clareza e robustez.
- **Detalhes:**
    ...
    - **Refatoração:** Ajustada a lógica no comando `automacao_documentos_ipiranga.py` para lidar com o novo dataclass `ExtractedCertificateData` retornado pela função `extract_certificate_data_from_filename`.
