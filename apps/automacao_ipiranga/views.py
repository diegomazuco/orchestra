    ...
    if request.method == "POST":
        try:
            ...
            if certificado_id is None:
                return JsonResponse(
                    {"status": "erro", "message": "ID do certificado não fornecido."},
                    status=400,
                )
            ...
            return JsonResponse(
                {
                    "status": "sucesso",
                    "message": f"Automação iniciada para o certificado ID: {certificado_id}",
                }
            )
        except json.JSONDecodeError:
            return JsonResponse(
                {"status": "erro", "message": "Requisição inválida. JSON esperado."},
                status=400,
            )
