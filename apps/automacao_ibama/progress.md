...
- **Ajuste da Espera por Elemento na Página de Login:** A estratégia de espera pela página de login no comando `process_portran_vehicles.py` foi alterada para aguardar a visibilidade do campo de usuário (`#codigoUsuario`) em vez de depender de `networkidle`, visando resolver o problema de timeout persistente.